...
(function () {
  ...
  if (!E || typeof E.registerModule !== "function") {
    console.error("[dda-poyc] EIKON core not found (registerModule missing)");
    return;
  }
  if (typeof E.apiFetch !== "function") {
    console.error("[dda-poyc] E.apiFetch missing (needed for authenticated requests)");
    return;
  }
  if (typeof E.el !== "function") {
    console.error("[dda-poyc] E.el missing");
    return;
  }
  ...
  E.registerModule({
    ...
    async render(ctx) {
      const root = ctx && ctx.mount;
      if (!root || typeof root.appendChild !== "function") {
        throw new TypeError("[dda-poyc] ctx.mount is not a DOM element");
      }
